...
class user_search {
    ...
    private function get_base_sql(string $fields): array {
        ...
        $joins = '';
        $wheres = 'u.deleted = 0 AND u.suspended = 0';
        $params = [];

        $context = $this->course->get_context();

        // Se NÃO for a busca global, aplicamos as travas de curso.
        if ($this->course->id != SITEID) {
            $ejoin = get_enrolled_join($context, 'u.id', true);
            $joins = $ejoin->joins;
            $wheres .= ' AND ' . $ejoin->wheres;
            $params = $ejoin->params;
            ...
            // Na busca Global, apenas garantimos que não listamos o usuário atual nem convidados.
            $wheres .= ' AND u.confirmed = 1 AND u.id <> :userid AND u.id <> :guestid';
