...
public static function get_preferences_raw() {
    ...
    foreach (course::get_by_user($user) as $course) {
        $result['courses'][] = [
            ...
            'groups' => $course->get_viewable_groups($user),
            'roles' => $course->get_viewable_roles($user),
        ];
    }
    ...
    public static function get_message_form() {
        ...
        $options = message_data::file_options($message->course);
        ...
        $PAGE->set_context($message->course->get_context());
        $options['autosave'] = false;
        $options['itemid'] = $data->draftitemid;
        ...
        $recitstub = '<script type="text/javascript">
            window.M = window.M || {};
            window.M.recit = window.M.recit || {
                htmlbootstrapeditor: {
        ...
        $javascript = preg_replace(
            '/Y\.on\s*\(\s*[\'"]domready[\'"]\s*,/i',
            '(function(evt, fn) { if (document.readyState === "complete" || document.readyState === "interactive") { fn(); } else { Y.on(evt, fn); } })("domready",',
            $javascript
        );

        $javascript = $recitstub . $javascript;
